...
CSMemers.client.on(Events.MessageCreate, async (message) => {
    if (message.author.id == config.users.bruv) {

        const messages = Message.readMessagesFile();

        if (message.reference && message.reference.messageId) {
            const parent = await message.channel.messages.fetch(message.reference.messageId);
            const parentMessage = new Message(parent.id, parent.author.id, parent.content, null);
            const newMessage = new Message(message.id, message.author.id, message.content, parentMessage);
            messages.push(newMessage);
        }
        else {
            const parent = (await message.channel.messages.fetch({limit: 2, before: message.id})).last();
            const parentMessage = new Message(parent.id, parent.author.id, parent.content, null);
            const newMessage = new Message(message.id, message.author.id, message.content, null);
            messages.push(newMessage);
        }

        Message.writeMessagesFile(messages);
    }
});
